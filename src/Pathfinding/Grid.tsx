...
export class Grid extends React.Component<GridProps, GridState> {
  constructor(props: any) {
    ...
    this.state = {
      ...
      start: [Math.floor(calculateCellsFromHeight(window.innerHeight) / 2), 2],
      interDest: [
        2,
      ...
      const repeat = setInterval(() => {
        if (i === visitedNodes.length) {
          ...
          if (visitedNodes[i].visited) {
            this.updateGrid(visitedNodes[i].row, visitedNodes[i].col, {
              visited: true,
            });
          } else {
            this.updateGrid(visitedNodes[i].row, visitedNodes[i].col, {
  ...
  makePath = (visitedNodes: GridNode[] | StarNode[]) => {
    ...
    for (const node of visitedNodes) {
      if (node.row === this.state.dest[0] && node.col === this.state.dest[1]) {
    ...
    if (targetNodeExists) {
      ...
      setTimeout(() => this.setState({ finished: true, running: false }), 1000);
    }
  };
  ...
  onClickMaze = () => {
    ...
    if (this.props.maze === "DFS") {
      maze = dfsMaze(this.state.grid);
    } else if (this.props.maze === "Prim") {
      maze = prim(this.state.grid);
    }
    for (let i = 0; i < this.state.grid.length; i++) {
      for (let j = 0; j < this.state.grid[0].length; j++) {
      ...
      <div className="main-section">
                ...
                {row.map((cell, colIdx) => (
                  ...
                  >
                    <div
                      ...
                      onPointerDown={() => this.pointerDown(rowIdx, colIdx)}
                      onPointerEnter={() => this.pointerEnter(rowIdx, colIdx)}
                      onPointerUp={() => this.pointerUp}
                    />
                  </td>
                ))}
        ...
        <div className="button-section">
          <button
            ...
            disabled={this.state.running || this.state.finished ? true : false}
          >
            Start
          </button>
          <button
...
const calculateCellsFromWidth = (width: number) => {
  return width > maxWidth ? Math.floor(maxWidth / 34) : Math.floor(width / 34);
};
