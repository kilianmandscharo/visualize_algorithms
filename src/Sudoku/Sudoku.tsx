...
export const Sudoku = (props: any) => {
  ...
  const [prevDisplayBoard, setPrevDisplayBoard] =
    useState(createDisplayBoard());
      ...
      if (errors.length === 0) {
        ...
        if (props.animation) {
          ...
          for (let i = 0; i < 9; i++) {
            const newRow = [];
            for (let j = 0; j < 9; j++) {
              newRow.push(makeDisplayCell(i, j, tempBoard[i][j], false, false));
            }
  ...
  const animateSolve = (iterations: any) => {
    let i = 0;
  ...
  return (
    <div>
      ...
      <div className="button-section">
        ...
        <button className="button" onClick={onClickReset} disabled={running}>
          {reset ? "Clear" : "Reset"}
        </button>
      </div>
    </div>
